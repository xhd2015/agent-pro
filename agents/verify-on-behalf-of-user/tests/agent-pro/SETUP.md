...
**Feature**: agent-pro exposes verify-on-behalf-of-user via knownSkills

```
go build cmd/agent-pro -> agent-pro skill verify-on-behalf-of-user --show
agent-pro skill --install -> SKILL.md + scripts/
```

## Preconditions

- `cmd/agent-pro/skill_cmd.go` registers `verify-on-behalf-of-user` with ExtraFiles.
