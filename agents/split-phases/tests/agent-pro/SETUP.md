...
**Feature**: agent-pro exposes split-phases via knownSkills registration

```
go build cmd/agent-pro -> agent-pro skill split-phases --show -> embedded SKILL.md
agent-pro skills -> lists split-phases with description
```

## Preconditions

- `cmd/agent-pro/skill_cmd.go` registers `split-phases` in `knownSkills`.
- `knownSkillNames()` includes `split-phases`.
