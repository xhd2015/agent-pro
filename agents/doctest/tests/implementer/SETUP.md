## Preconditions
- The repository root contains `agents/doctest` and `cmd/fake-codex`.
- Tests are executed by the doc-style test runner from this tree.
- Agent invocations use `fake-codex` to avoid real LLM calls.

## Steps
1. Build `agents/doctest` and `cmd/fake-codex` into temp binaries.
2. Copy doctest as `yield-pending-questions` for ypq tests.
3. Configure env vars so agent implement uses fake-codex.
4. Provide helper functions for writing mock configs and inspecting session metadata.
...
func Setup(t *testing.T, req *Request) error {
    ...
    buildFC := exec.Command("go", "build", "-o", fakeCodex, "./cmd/fake-codex")
    buildFC.Dir = req.WorkDir
