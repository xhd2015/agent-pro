...
//
// The followup agent keeps the conversation in clarification phase without
// moving to implementation. It auto-detects the parent agent runner
// (opencode, pi, codex, crush) and delegates execution via the subagent
// package.
//
// Usage:
//
//	followup [OPTIONS] <prompt>
//
// Options:
//
...
const followupPrefix = "followup(clarification phase, don't go to implementation phase): "

var help = `
Usage: followup [OPTIONS] <prompt>

A clarification-phase followup agent. Stays in discussion mode without
moving to implementation. Delegates to the parent agent runner
(auto-detected) for LLM calls.
...
Options:
  ...
  --list-sessions          list all followup sessions
  --session-base <dir>     override sessions directory
  -h, --help               show this help
`

const defaultModelEnv = "AGENT_PRO_SUBAGENT_FOLLOWUP_MODEL"

// Config holds all customizable configuration for the followup agent.
...
// Run runs the followup agent programmatically with the given config.
