...
**Feature**: `tty send` sends prompt via WebSocket and captures assistant response

```
# connect WS -> inject prompt with Ctrl+U prefix -> read response -> extract assistant text -> store event
agent-run tty send session-1 "hello" -> resolveTerminal -> WS connect -> inject Ctrl+U + "hello" + \r -> capture -> store events
```
...
3. `req.Args` = `["tty", "send", "session-1", "hello"]`.
