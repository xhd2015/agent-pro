...
- Exit code ≠ 0.
- Error indicates `--detach` and `--open` mutually exclusive (not unknown flag).

## Errors

- Mutual exclusion between `--detach` and `--open` on resume.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode == 0 {
		t.Fatalf("expected non-zero exit for resume --detach --open; stdout:\n%s\nstderr:\n%s",
	...
	if !(hasDetach && hasOpen && hasExclusive) {
		t.Fatalf("stderr/stdout should explain resume --detach vs --open exclusivity:\nstderr:\n%s\nstdout:\n%s",
