...
**Feature**: `agent-run run --auto-session-id` generates a shared storage/TTY session id
...
- Session cache dir: `$TMPDIR/agent-run-auto-session-id-doctest-<DOCTEST_SESSION_ID>/`.
...
func sessionCacheDir() string {
	return filepath.Join(os.TempDir(), "agent-run-auto-session-id-doctest-"+DOCTEST_SESSION_ID)
}
