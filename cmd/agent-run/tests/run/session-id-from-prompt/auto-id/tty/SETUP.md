...
**Feature**: session-id-from-prompt with TTY runner uses the same id for storage and registry

```
agent-run run --agent-runner grok-tty --session-id-from-prompt "prompt"
