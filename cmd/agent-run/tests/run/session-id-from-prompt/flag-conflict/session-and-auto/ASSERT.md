...
- Error clearly indicates mutual exclusion / incompatibility of `--session` and
  `--session-id-from-prompt` (not merely "unrecognized flag").
- Both flag names should appear in the error surface, or exclusivity wording
  referencing session-id-from-prompt together with session.

## Errors

- Mutual exclusion between `--session` and `--session-id-from-prompt`.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode == 0 {
		t.Fatalf("expected non-zero exit when --session and --session-id-from-prompt both set; stdout:\n%s\nstderr:\n%s",
	...
	if strings.Contains(errText, "unrecognized flag") || strings.Contains(errText, "unknown flag") {
		t.Fatalf("want mutual-exclusion error for --session + --session-id-from-prompt, got unrecognized/unknown flag:\n%s",
			resp.Stderr)
	}

	hasAuto := strings.Contains(errText, "session-id-from-prompt")
	// Require the explicit --session flag token (not the substring inside session-id-from-prompt).
	...
	if !(hasAuto && (hasSessionFlag || hasExclusiveWording)) {
		t.Fatalf("stderr/stdout should explain --session vs --session-id-from-prompt mutual exclusion:\nstderr:\n%s\nstdout:\n%s",
