...
- Exit code 0.
- Reuses agent-run session id `takeover-codex-mapped-resume-s1` (stdout/script/meta).
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if rsid, _ := meta["runner_session_id"].(string); rsid != takeoverCodexFixtureSessionID {
		t.Fatalf("runner_session_id=%q want %s", rsid, takeoverCodexFixtureSessionID)
	}
	if runner, _ := meta["runner"].(string); runner != "" && runner != "codex-tty" && runner != "codex" {
		t.Fatalf("mapped meta runner=%q want codex-tty", runner)
	}
	...
	for _, id := range listAgentSessionIDs(t, req.Home) {
		...
		if rsid, _ := m["runner_session_id"].(string); rsid == takeoverCodexFixtureSessionID {
			mapped++
		}
	}
	if mapped != 1 {
