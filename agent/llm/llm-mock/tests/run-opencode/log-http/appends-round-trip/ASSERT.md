...
- At least one line has `request.path` containing `/v1/chat/completions` (not `/v1/responses`).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	for i, rec := range records {
		...
		if strings.Contains(path, "/v1/responses") {
			t.Fatalf("line %d: unexpected /v1/responses path (want chat/completions): %s", i+1, path)
		}
	...
	if !found {
		t.Fatalf("no log line with request.path containing /v1/chat/completions and status 200 in:\n%s",
			resp.LogHTTPContent)
	}
}
```