...
- Exactly one `tool_calls` with `function.name` = `bash`.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !ok || len(toolCalls) != 1 {
		t.Fatalf("expected exactly 1 tool_call, got %v", message["tool_calls"])
	}
