...
**Feature**: send via --tab (ResolveFromTab → short-circuit)
...
const (
	fixtureTabSendSessionID = "019f283b-bbbb-7bbb-bbbb-bbbbbbbbbbbb"
	pidTabSendGrok          = 8300
)
...
func Setup(t *testing.T, d *session.Doctest, req *Request) error {
	_ = t
	_ = d
	req.SessionID = fixtureTabSendSessionID
	return nil
}

func seedSendTabWindow(req *Request) {
	...
	req.Procs = []sessions.FocusProc{
		{PID: pidTabSendGrok, PPID: 1, TTY: "/dev/ttys102", Cmd: "/usr/local/bin/grok"},
	}
	if req.OpenFiles == nil {
		req.OpenFiles = map[int][]string{}
	}
	req.OpenFiles[pidTabSendGrok] = []string{grokSendPath(fixtureTabSendSessionID)}
}
```
