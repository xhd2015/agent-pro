...
**Feature**: open via --tab / --tab-index (ResolveFromTab → short-circuit focus)
...
const (
	fixtureTabOpenSessionID = "019f283b-dddd-7ddd-dddd-dddddddddddd"
	pidTabOpenGrok          = 8200
)
...
func Setup(t *testing.T, d *session.Doctest, req *Request) error {
	_ = t
	_ = d
	req.SessionID = fixtureTabOpenSessionID
	return nil
}

func seedOpenTabWindow(req *Request) {
	...
	req.Procs = []sessions.FocusProc{
		{PID: pidTabOpenGrok, PPID: 1, TTY: "/dev/ttys102", Cmd: "/usr/local/bin/grok"},
	}
	if req.OpenFiles == nil {
		req.OpenFiles = map[int][]string{}
	}
	req.OpenFiles[pidTabOpenGrok] = []string{grokOpenPath(fixtureTabOpenSessionID)}
}
```
