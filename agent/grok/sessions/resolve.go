...
Options:
  ...
  --details         also include session title/cwd/kind on stdout (or in --json)
...
// enrichResolveSessionDetails loads title/cwd/kind/last_active from summary.json.
// Soft: Find misses leave details unchanged (resolve still succeeds).
func enrichResolveSessionDetails(opts *ResolveOpts, d *ResolveDetails) {
	...
	sess, err := Find(home, d.SessionID)
	if err != nil {
		return
	}
	d.Title = strings.TrimSpace(sess.Title)
	d.CWD = strings.TrimSpace(sess.CWD)
	d.Kind = sessionKindOrMain(sess)
	if !sess.LastActiveAt.IsZero() {
		d.LastActive = sess.LastActiveAt.In(time.Local).Format(time.RFC3339)
	}
}
...
func resolveHasSessionDetails(d ResolveDetails) bool {
	return d.Kind != "" || d.Title != "" || d.CWD != "" || d.LastActive != ""
}
