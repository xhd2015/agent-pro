...
const OpenHelp = `Usage: agent-pro codex session open (<session-id> | --tab SEL | --tab-index N) [OPTIONS]
...
func RunOpen(args []string, stdout, stderr io.Writer, codexHome string, opts *OpenOpts) error {
	...
	result, err := Open(codexHome, sessionID, &runOpts)
	if err != nil {
		return err
	}
