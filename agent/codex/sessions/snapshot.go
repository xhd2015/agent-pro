...
const SnapshotHelp = `Usage: agent-pro codex session snapshot (<session-id> | --tab SEL | --tab-index N) [OPTIONS]
...
(same as: kool iterm2 contents <iterm-uuid>). Bare codex (not under agent-run)
always uses iTerm.
...
func (f *SnapshotFake) SnapshotOpts() *SnapshotOpts {
	...
	if f.AgentRunEnabled || f.AgentRunByID != nil || f.AgentRunErr != nil {
		opts.AgentRunSnapshot = func(codexSessionID string) (*AgentRunSnapshotResult, error) {
			f.AgentRunCalls = append(f.AgentRunCalls, codexSessionID)
	...
	if opts.TabFrom != nil {
		...
		if selErr != nil {
			msg := strings.ReplaceAll(selErr.Error(),
				...
				"agent-pro codex session snapshot")
...
func RunSnapshot(args []string, stdout, stderr io.Writer, codexHome string, opts *SnapshotOpts) error {
	...
	result, err := Snapshot(codexHome, sessionID, &runOpts)
	if err != nil {
		return err
	}
