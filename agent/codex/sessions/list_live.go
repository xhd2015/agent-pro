...
const ListLiveHelp = `Usage: agent-pro codex session list-live [OPTIONS]

List Codex session ids that currently have a hosting iTerm2 tab.
Discovery: live codex processes (open-files hard hit) → TTY → iTerm match.
...
// ListLive returns Codex sessions that have at least one hosting iTerm tab.
...
func ListLive(opts *ListLiveOpts) ([]LiveHostingRow, error) {
	...
	sids, metaIndex, err := discoverLiveCodexSessions(opts)
	if err != nil {
		return nil, err
	}
...
func withSharedListLiveProbes(opts *ListLiveOpts) *ListLiveOpts {
	...
	if productionLsof {
		// One bulk lsof for all codex runners; seed the per-pid cache.
		var pids []int
		for _, p := range procs {
	...
	if opts.ListITerm == nil && opts.PaneByTTY == nil {
		...
		// One TTY-targeted AppleScript for codex-host TTYs; title/cwd from open-file summary.
		ttys := codexHostingTTYs(procs)
...
// codexHostingTTYs returns normalized TTYs for live codex runners (including
// ancestor/descendant TTYs), used for one FindSessionsByTTY inventory.
func codexHostingTTYs(procs []FocusProc) []string {
...
// discoverLiveCodexSessions finds live Codex sids via open-file hard hits and
// loads TITLE/CWD from summary.json beside each hit (remapped into CodexHome
// when set). No WalkDir over the sessions tree.
func discoverLiveCodexSessions(opts *ListLiveOpts) ([]string, map[string]liveSessionMeta, error) {
	listProcs := opts.ListProcs
	if listProcs == nil {
		listProcs = listLiveFocusProcs
	}
	...
	for _, p := range listProcs() {
		...
		for _, path := range lsof(p.PID) {
			...
			if _, has := meta[sid]; !has {
				if m, ok := metaFromCodexOpenPath(path, opts.CodexHome); ok {
					meta[sid] = m
				}
			}
		}
...
// metaFromCodexOpenPath reads title/cwd from a hard-hit Codex rollout path.
// When codexHome is set, remaps …/.codex/sessions/<rel> → codexHome/sessions/<rel>
// so injected fixtures (fixture paths + temp CodexHome) still resolve.
// Streams the file (large-token scanner) and stops once cwd + title are known.
func metaFromCodexOpenPath(openPath, codexHome string) (liveSessionMeta, bool) {
	path := remapCodexRolloutPath(openPath, codexHome)
	...
	var meta liveSessionMeta
	gotMeta := false
	sc := newRolloutScanner(f)
	for sc.Scan() {
		line := sc.Text()
		if !gotMeta {
			if m, ok := parseSessionMeta(line); ok {
				meta.Cwd = strings.TrimSpace(m.CWD)
				gotMeta = true
			}
		}
		if meta.Title == "" {
		...
		if gotMeta && meta.Title != "" {
			break
		}
	}
	if err := sc.Err(); err != nil && !gotMeta && meta.Title == "" {
		return liveSessionMeta{}, false
	}
	if !gotMeta && meta.Title == "" {
		return liveSessionMeta{}, false
...
func remapCodexRolloutPath(openPath, codexHome string) string {
	codexHome = strings.TrimSpace(codexHome)
	if codexHome == "" {
		return openPath
	}
	slash := filepath.ToSlash(openPath)
	...
	return filepath.Join(codexHome, "sessions", filepath.FromSlash(rel))
}
