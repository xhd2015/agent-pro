...
const ResolveHelp = `Usage: agent-pro codex session resolve [OPTIONS]
...
func RunResolve(args []string, opts *ResolveOpts) error {
	...
	if tabSet || tabIndexSet {
		...
		anc, ok := procresolve.FindAncestorCodex(startPID, resolveOpts)
		result, err := procresolve.ResolveFromAncestorsCodex(startPID, resolveOpts)
		if err != nil {
			return err
		}
		if !ok {
			return fmt.Errorf("no ancestor codex")
		}
		if result == nil || strings.TrimSpace(result.SessionID) == "" {
			return fmt.Errorf("session not resolved")
		}
		if result.Kind != "" && result.Kind != "codex" {
			return fmt.Errorf("session not resolved")
		}
