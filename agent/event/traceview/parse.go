...
func ParseMessages(lines []string, createdAt string) []AgentTraceMessage {
	baseMs := parseTraceCreatedAtMs(createdAt)
	messages := make([]AgentTraceMessage, 0, len(lines))
	for i, line := range lines {
		if parseTraceLine == nil {
			continue
		}
		parsed, ok := parseTraceLine(json.RawMessage(line))
		if !ok {
			continue
		}
		ts := baseMs + int64(i)
...
func isSubtypeStarted(s ActivitySubtype) bool   { return s == SubtypeStarted }
func isSubtypeCompleted(s ActivitySubtype) bool { return s == SubtypeCompleted }

func appendTraceToolMessage(messages *[]AgentTraceMessage, event AgentTraceActivity, ts int64) {
	if isSubtypeStarted(event.Subtype) {
		*messages = append(*messages, AgentTraceMessage{
			Role:      RoleToolCall,
			Content:   "",
			ToolCall:  &event,
			StartedAt: &ts,
	...
	for i := len(*messages) - 1; i >= 0; i-- {
		msg := &(*messages)[i]
		if msg.Role != RoleToolCall || msg.ToolCall == nil || msg.FinishedAt != nil {
			continue
		}
