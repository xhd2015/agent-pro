...
**Feature**: explain config preferences CLI (e2e binary)
...
- Session cache under `$TMPDIR/explain-config-doctest-<session>/`.
...
4. Leaves set `Args` (and optionally seed `config.json` / `WorkingAgentPath`).
...
func writeConfigJSON(t *testing.T, configHome, body string) {
	t.Helper()
...
func sessionCacheDir(sessionID string) string {
	return filepath.Join(os.TempDir(), "explain-config-doctest-"+sessionID)
}
