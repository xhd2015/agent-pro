...
- Persisted session `agent_runner` is `opencode` (CLI wins over config codex).
- No config `notice:` on stderr (flag supplied the runner).
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if strings.Contains(resp.Stderr, "notice:") {
		t.Fatalf("CLI flag must not emit config notice:\n%s", resp.Stderr)
	}
}
```
