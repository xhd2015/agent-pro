...
**Feature**: channel name/ID resolution reflected in Sending line for send

```
slack-msg send --channel CH -> resolve (ID passthrough | knownChannels | conversations.list) -> Sending line shows resolved ID
```
...
3. Assert stdout `Sending to channel=RESOLVED` and successful send.
