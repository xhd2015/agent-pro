...
- Exit code 0.
- Stderr empty.
- Same topic body contract as `help-then-topic` (flag order must not change content).
- Stdout ends with trailing `\n`.
- Contains: `api.slack.com/apps`, `groups:read`, reinstall, botToken/SLACK_BOT_TOKEN/config, retry.
