...
```
slack-listen listen --config <repo>/slack-config.json -> connects -> logs config path
```
...
func Setup(t *testing.T, req *Request) error {
	// Leaf relies on integration grouping for config/tokens; mark live probe explicitly.
