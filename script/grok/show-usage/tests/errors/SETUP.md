...
**Feature**: grok-show-usage surfaces failures on stderr with non-zero exit

```
# failure condition -> non-zero exit, stderr explains reason, stdout empty or partial
grok-show-usage -> error path -> exit != 0
doctest <- stderr: grok / timeout / parse message
```

## Preconditions

- Each error leaf isolates one failure mode: missing grok, timeout, or parse error.
- Success stdout (two clean lines) must not be produced.
...
- `grok-not-found` uses minimal PATH and omits `GROK_SHOW_USAGE_COMMAND`.
...
func Setup(t *testing.T, req *Request) error {
	if req.Bin == "" {
		t.Fatalf("errors setup: grok-show-usage binary not built (root Setup skipped?)")
	}
	return nil
}
...
func assertStdoutNotSuccessLines(t *testing.T, resp *Response) {
	...
	if strings.Contains(trimmed, "Weekly limit:") && strings.Contains(trimmed, "Next reset:") {
