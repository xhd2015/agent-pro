...
const help = `
Usage: dev [agent-traces options]

Start agent-traces in dev mode.
This wrapper runs:

  go run ./ --dev [agent-traces options...]

Examples:
  go run ./script/dev
  go run ./script/dev --port 9899
  go run ./script/dev --route-prefix agent-traces
