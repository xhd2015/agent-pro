...
const listenHelpText = `slack-listen: Slack Socket Mode inbound bridge.
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: slack-listen listen [options]")
		os.Exit(1)
	}

	switch os.Args[1] {
	case "listen":
		if err := runListenCommand(os.Args[2:]); err != nil {
...
func runListenCommand(args []string) error {
	...
	remain, err := lessflags.String("--bot-token", &botTokenFlag).
	...
	botToken := slackutil.ResolveBotToken(flagString(botTokenFlag), "SLACK_BOT_TOKEN", cfg)
	if botToken == "" {
