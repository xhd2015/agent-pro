...
const helpText = `slack-send: send a message via Slack Web API.
...
func main() {
	var tokenFlag, channelFlag, configFlag *string
	...
	respChannel, ts, err := api.PostMessage(resolvedChannel, slack.MsgOptionText(message, false))
	if err != nil {
		fmt.Fprintf(os.Stderr, "send failed: %v\n", err)
		os.Exit(1)
	}
